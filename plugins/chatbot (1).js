...
if (m.fromMe) return
if (m.chat === "120363399175402285@newsletter") return; 
if (m.chat === "120363399175402285@newsletter") return;
...
*❇️ ${await tr("PayPal:")}* 
https://github.com/Fer280809/Asta_bot
...
> 「 『𝕬𝖘𝖙𝖆-𝕭𝖔𝖙』」`, m, {contextInfo: {externalAdReply :{ mediaUrl: null, mediaType: 1, description: null, title: `Hola ${name} 👋`, body: wm, previewType: 0, thumbnail: img.getRandom(), sourceUrl: redes.getRandom()}}})
}
try {
...
query = m.text.replace(prefixRegex, '').replace(/(bot|Bot|simsimi|simi|astabot|asta)/i, '').trim(); 
} else if (hasKeywordWithoutPrefix) {
const keywordRegex = /^(bot|Bot|simsimi|simi|Astabot|asta)\s+/i;
...
let syms1 = await fetch('https://raw.githubusercontent.com/Fer280809/Asta_bot/main/src/text-chatgpt.txt').then(v => v.text());
